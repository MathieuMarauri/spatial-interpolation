...
A random field, or random function, is simply a set of random variables. We consider each value $z_1, \dots, z_n$ to be observations of the random variables $Z_1, \dots, Z_n$. $Z(s) = (Z_1(s), \dots, Z_n(s))$ is a random function, or random field. $s$ represents a position in space. 
...
It states that the joint distribution of $Z(s_i)$ is the same as the one of $Z(s_i + h)$ where $h$ indicates a translation. It does not require the first moment nor the second one to exist.

##### The second-order stationarity

It is weaker than the first one. $Z(s)$ is said to be second-order stationary if the first 2 moments exists and are invariant by translation.

Therefore for all $s$, $s'$ and $h$ we have:

$$
\begin{gathered}
Cov(Z(s), Z(s')) = Cov(Z(s+h), Z(s'+h)) \\
\mathbb{E}[Z(s)] = \mathbb{E}[Z(s+h)]
\end{gathered}
$$
...
$$
\mathbb{E}[Z(s+h) - Z(h)] = 0 \\
V(Z(s+h), Z(s)) = 2\gamma(h)
$$
The condition is no longer on the random function $Z(s)$ itself but rather on the increments $Z(s+h) - Z(h)$. The function $\gamma(h)$ is the semi-variogram function. 
...
It is a function that only depends on $h$, the distance between 2 points, and that describes the spatial dependence of a random field. It measures how much 2 locations separated by $h$ will vary in their $z$ value. 

Now that some basic concepts have been defined, let's dive into the kriging method. We will be defining ordinary Kriging.

## How it works

Recall that our goal is to find the weights of this equation $\widehat{z_0} = \sum_{i = 1}^{n}\lambda_i z_i$. We want to find the weights $\lambda_i$ required to predict the value $z_0$. We want our estimate to be unbiased. In other words we want our predictions to be exact, on average. Mathematically we want: 
...
The difference between 2 unbiased estimator is their variance. In order for our estimator to be a good one we also need its variance to be as low as possible. 
...
$$
\gamma(h) = \frac{1}{2|N_h|} \sum_{i=1}^{|N_h|}\left(Z(s_{i+h})-Z(s_i)\right)^2
$$
...
To compute the empirical variogram we must start by calculating the value $\left(Z(s_{i+h})-Z(s_i)\right)^2$ for every pair of points $(s_i, s_j), i,j = 1, \dots,n$.
...
The resulting points are visualised in the following figure. There is one point per group defined (15 in this case). The x-value of each point is the center of the distance bin, the y-value is the average of the $\left(Z(s_{i+h})-Z(s_i)\right)^2$ value for each point in the bin. 
...
* the nugget $\gamma_0$ is the semi-variance when $h \to 0$. It represents the measurement error. 
...
$$
We want the parameters $\gamma_0, \sigma, \phi$ that minimise the loss. There are simply obtained with the `optim()` function.
...
The obtained parameters, the object `exponential_param`, can then be used to build the semi-variogram function.
...
vario_func <- function(x) {
  return(ifelse(
    test = x == 0,
    yes = 0,
    no = exponential_param[3] + exponential_param[1] * (1 - exp(-x / exponential_param[2]))
  ))
}
```
...
#### Defining the semi-variogram with igh-level functions

All of the above can be done with functions from the `geoR` package. Here is an example code that you might do if you were to build a semi-variogram.

```{r variog-code, echo = TRUE}
...
variogram <- variog(
  ...
  breaks = c(seq(0, 30, 5), seq(30, 70, 10)) # limits of the bins to create
)
plot(variogram)
